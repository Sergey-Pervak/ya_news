...
3 Если комментарий содержит запрещённые слова, он не будет
опубликован.
4 Авторизованный пользователь может редактировать или удалять
свои комментарии.
5 Авторизованный пользователь не может редактировать или удалять
чужие комментарии.
"""
...
class TestCommentCreation(TestCase):
    ...
    # Убедимся, что анонимный пользователь не может создать комментарий.
    def test_anonymous_user_cant_create_comment(self):
        # Совершаем запрос от анонимного клиента, в POST-запросе отправляем
        # предварительно подготовленные данные формы с текстом комментария.
        # Добавил от себя: ищем ком. до
        comments_count_before = Comment.objects.count()
        self.client.post(self.url, data=self.form_data)
        # Считаем количество комментариев.
        # Добавил от себя: ищем ком. после
        comments_count_after = Comment.objects.count()
        # Добавил от себя: ищем разницу до и после
        # Ожидаем, что комментариев в базе нет - сравниваем с нулём.
        comments_count = comments_count_after - comments_count_before
...
class TestCommentEditDelete(TestCase):
    ...
    # Проверим, что, редактирование комментария недоступно для
